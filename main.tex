...
Este estudio propone el siguiente árbol de problemas (véase Figura \ref{fig:arbol}) con el fin de identificar las causas y consecuencias que desencadena el tema del presente de forma precisa.
	...
	\begin{figure}[H]\centering
	\includegraphics[width=1.4\textwidth]{resources/img/arbol_de_problemas.png}
...
árbol de problemas (figura \ref{fig:arbol}). Para atender la estrctura de los problemas
presentados previamente, se genera de forma análoga un árbol de objetivos (vease figura ---).
Bajo esta metodología podemos guarantizar que el planteamiento de soluciones está alineado a la identificación
de problemáticas de forma directa.
...
Usando como referencia el árbol de objetivos (vease ...) de la sección anterior, podemos plantear el objectivo general
de la siguiente forma:

\forceindent \textit{Reducir la taza de accidentes viales mediante el análisis de información relevante y selección de políticas públicas adecuadas.}


Con este objetivo se pretente analizar la relación que existe entre los accidentes automovilísticos en la zona metropolitana de Guadalajara,
sus causas y efectos. También las posibilidades que existen de prevención mediante políticas
públicas que impacten de manera positiva y beneficien a la sociedad. Asimismo,
se propone la evaluación de datos verídicos que nos permitan detectar fenómenos sociales y a su vez,
poder plantear una solución que sea benefactora para los locatarios de la comunidad, además de poder cumplir el
objetivo de salvar vidas y poder contribuir al problema de tráfico vial.


\subsection{Objetivos específico}\label{subsec:specific-objs}

Entre los objetivos específicos evidenciados por la parte inferior del árbol de objetivos (vease figura ...)
destacan los siguientes:

\forceindent \textit{agregar objetivo}
